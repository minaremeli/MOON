...
class FedAvg(Strategy):
    ...
    def weighted_acc_avg(self, results: List[Tuple[int, float, Optional[float]]]) -> float:
        """Aggregate evaluation results obtained from multiple clients."""
        num_total_evaluation_examples = sum(
            [num_examples for num_examples, _, _ in results]
        )
        weighted_accs = [num_examples * acc for num_examples, _, acc in results]
        return sum(weighted_accs) / num_total_evaluation_examples
    ...
    def aggregate_evaluate(
        ...
        if not self.accept_failures and failures:
            return None, {}
        acc_aggregated = self.weighted_acc_avg(
            [
                (
